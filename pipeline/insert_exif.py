...
class ExifToolSession:
    ...
    def resolve_exiftool_path():
        env_path = os.getenv("MOTHBOT_EXIFTOOL_PATH")
        if env_path:
            return env_path

        # Prefer bundled paths when frozen by PyInstaller.
        ...
        if platform.system() == "Windows":
            candidates.extend(
                [
                    module_dir.parent / "exiftool-13.32_64" / "exiftool.exe",
                    module_dir.parent / "exiftool-13.32_64" / "exiftool",
                    meipass / "exiftool-13.32_64" / "exiftool.exe",
    ...
    def __init__(self, exiftool_path=None):
        if exiftool_path is None:
            exiftool_path = self.resolve_exiftool_path()
        ...
        try:
            ...
            raise RuntimeError(
                "ExifTool executable not found. Set MOTHBOT_EXIFTOOL_PATH "
                "or install exiftool in PATH."
            ) from exc
