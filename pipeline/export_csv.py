...
def get_deepest_classification(
...
def get_gbif_info_online(rank, name):
    try:
        ...
        if search_results["results"]:
            ...
            if scientific_name:
                scientific_name = (
                    unicodedata.normalize("NFKD", scientific_name)
                    .encode("ascii", "ignore")
                    .decode("utf-8")
                )
            if common_name:
                common_name = (
                    unicodedata.normalize("NFKD", common_name)
                    .encode("ascii", "ignore")
                    .decode("utf-8")
                )
...
def find_deepest_taxon_info(taxa_list, taxa_values, taxa_lookup):
    ...
    for rank, value in valid_ranks:
        ...
        filtered_rows = taxa_lookup[
            taxa_lookup["taxonRank"].str.lower() == rank.lower()
        ]
        ...
        if not matched_row.empty:
            ...
            scientific_name = matched_row["scientificName"].values[
                0
            ]  # GBIF Species lists don't apparently have a common name
...
def json_to_csv(input_path, taxa_list_path):
    ...
    try:
        ...
        print(
            "Could not find fiftyone dataset's json path, maybe try a different folder?"
        )
        return
    ...
    with open(input_path + "/" + output_file, "w", newline="") as csvfile:
        ...
        for sample in data["samples"]:
            ...
            taxon_id, scientificName = find_deepest_taxon_info(
                TAXA, taxa_values, taxa_lookup
            )

            uniquedatasetID = create_uniquedatasetID(
                sample["deployment_name"], sample["_dataset_id"]
            )

            # Adjust date and timestamp based on the UTC offset
            formattedUTC_dateTime = format_datetime_with_utc_offset(
                date, timestamp, float(sample["UTC"])
            )
...
def run(input_path, taxa_csv):
    global INPUT_PATH, TAXA_LIST_PATH
    INPUT_PATH = input_path
    TAXA_LIST_PATH = taxa_csv
    json_to_csv(input_path, taxa_csv)


# This code will only run if this script is executed directly
if __name__ == "__main__":
    args = parse_args()
    run(args.input_path, args.taxa_csv)
