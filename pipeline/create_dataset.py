...
from pipeline.export_csv import json_to_csv
...
def load_anylabeling_data(
    json_path,
):  # TODO load METADATA STRAIGHT FROM CSV - METADATA_PATH - Maybe metadata gets loaded into its own 51 thing via the WHOLe dataset?
...
def write_taxonomy_with_exiv2_cli(image_path, taxonomic_list):
    ...
    try:
        ...
        print(
            " exiv2 CLI tool not found. Make sure it's installed and in your system PATH."
        )  # cannot do ❌
    ...
    for item in taxonomic_list:
        if "_" in item:
            ...
            subject_keywords.append(
                f"taxonomy:{level.lower()}={value.replace('_', ' ')}"
            )
...
def write_taxonomy_with_naturtag_old(
    image_path, taxonomic_list, include_common_names=False
):
    """
...
def write_taxonomy_with_naturtag(
    image_path, taxonomic_list, include_common_names=False
):
...
class ExifToolSession:
    ...
    def add_taxonomy_with_exiftool(self, full_patch_path, taxonomic_list):
        ...
        match = re.search(
            r"_(\d{4})_(\d{2})_(\d{2})__?(\d{2})_(\d{2})_(\d{2})", filename
        )
        if match:
        ...
        args.extend(
            ["-overwrite_original", "-fast2", str(full_patch_path), "-execute\n"]
        )
...
def create_sample(
    image_path, labels, image_height, image_width, metadata, detection_creator
):  # skipping exif tagger for now, tagger):
    ...
    sample["datasetcollection"] = metadata.get(
        "dataset", ""
    )  # call it a dataset collection here to not confuse with the fiftyonedataset
    ...
    for label in labels:
        ...
        filtered_dict = {
            key: value for key, value in label.items() if key in desired_keys
        }

        # Check for unwanted values
        unwanted_values = {"error", "ERROR", "Error"}
        if any(
            any(u.lower() in value.lower() for u in unwanted_values)
            ...
            taxonomic_list = [
                f"{key.upper()}_{value}" for key, value in filtered_dict.items()
            ]

        full_patch_path = Path(
            INPUT_PATH + "/" + the_patch_path
        ...
        if (
            shape_type == "rotation"
        ):  # Todo - these should be handled as a polygon in 51, because they only have regular rects they call "Detections" but we have rotated rects (that should be stored as polylines via polyline.fromrotatedboundingbox https://docs.voxel51.com/user_guide/using_datasets.html#rotated-bounding-boxes)
    ...
    sample["creature_detections"] = fol.Detections(
        detections=detections_list
    )  # TODO - give this an appropriate name
...
def generate_patch_dataset(
    ...
    for sample in dataset.iter_samples(progress=True):
        filename = os.path.basename(
            sample.filepath
        )  # this is just the basename that it stores!
        ...
        for detection in detections:
            ...
            inferred_patchfullpath = (
                Path(patch_folder_path) / f"{inferred_patchfilename}"
            )
...
def add_gps_exif(input_path, output_path, lat, lng, altitude=None):
    ...
    if gps_existing.get(piexif.GPSIFD.GPSLatitude) and gps_existing.get(
...
def generate_patch_thumbnails_orig(
    dataset, output_dir=INPUT_PATH + "/patches", target_size=(1024, -1)
):
    """
    Generates thumbnails for images in a FiftyOne dataset, skipping existing ones.
    ...
    for sample in dataset.iter_samples(progress=True):
        ...
        for detection in detections:
            ...
            if (
                not os.path.exists(patchfullpath)
                and SKIP_EXISTING_THUMBNAIL_PATCHES == False
            ):  # skip thumbs already generated unless we specifically target to overwrite them each time
...
def run(input_path, taxa_csv, metadata_path):
    global INPUT_PATH, TAXA_LIST_PATH, METADATA_PATH, UTC_OFFSET
    INPUT_PATH = input_path
    TAXA_LIST_PATH = taxa_csv
    ...
    for image_path, json_path in hu_pairs:
        ...
        labels, image_height, image_width, notmetadata, detection_creator = (
            load_anylabeling_data(json_path)
        )
    ...
    print(
        f"{bcolors.OKGREEN}The app is running, open your browser to use{bcolors.ENDC}"
    )
...
if __name__ == "__main__":
    args = parse_args()
    run(args.input_path, args.taxa_csv, args.metadata)
